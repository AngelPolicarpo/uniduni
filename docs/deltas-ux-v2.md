...
| **Em aberto, e declarado** | **Onde a DM mora na navegação** e **como a notificação dela é configurada** são **B63**, e não se derivam de §31: a primeira é escolha de arquitetura de informação e a segunda esbarra em `settings.setNotifications` ser por comunidade, que uma DM não tem. As duas propostas estão escritas em B63. **Emenda de 2026-09-09:** as duas foram decididas e esta linha ficou para trás — `backend-v2.md` §31.16 (emenda de 2026-09-04) ratificou a conversa como entrada no topo do rail (a) e a configurou com o flag global mais o **mudo por conversa** (b), e `frontend.md` §10 3.1a (2026-09-09) estendeu os dois ao retorno sonoro. Não há nível por conversa, e a ausência é decidida: uma conversa não é uma comunidade. O resto de U-33 nunca dependeu delas. `lag` e as listas de `partialInterpretation` chegam à UI como `0` e vazias enquanto a fonte não existir (§105.7) — a superfície **não anuncia** número que ninguém mediu. |
...
**Emenda de 2026-09-09 — RBAC, moderação, destruição de identidade e integridade de configuração (Fase 10 / §8.6, §15.3, §18.6, U-02, U-15).**
1. **Destruição criptográfica no logout (`AccountIdentityTab` / §18.6, L-25):** "Sair desta identidade" agora invoca obrigatoriamente o comando IPC `api.identityWipe()` (`identity.wipe`, com diálogo nativo de confirmação) antes de purgar o estado em memória, garantindo a destruição do banco SQLite local e da chave privada Ed25519 em disco.
2. **Contagem e limites em code points nos formulários de configuração (§8.6, §13):** O componente `TextArea` passa a suportar a propriedade `limiteCp` com clamp por escalares Unicode (`cortarCodePoints`). Formulários de criação de comunidade (`CreateCommunityModal`), identidade de comunidade (`CommunityIdentitySection`), nome de cargo (`RoleEditor`) e motivo de moderação (`ModerationDialog`) foram migrados para validação e contagem estrita em code points, eliminando falso `E_VALIDATION` em emojis e evitando truncagem no meio de pares substitutos UTF-16.
3. **Modo chamada no painel de membros (`MembersPanel` / §8 1.4):** O painel de membros agora computa os participantes da chamada ativa na comunidade corrente (`inCallParticipantIds`) e repassa a propriedade `inCall` tanto para o `MemberContextMenu` quanto para o `ProfilePopover`, restaurando as ações "Silenciar nesta chamada" e o controle individual de volume.
4. **Resiliência a cargos desconhecidos no Roster (`MembersPanel` / R-3):** Membros cujos cargos ainda não foram sincronizados ou que possuam referências residuais a cargos removidos deixam de ser descartados silenciosamente e passam a ser agrupados sob o cargo base (`cargoBase`) da comunidade, permanecendo visíveis e pesquisáveis.
5. **Reconciliação de abas privilegiadas (`CommunitySettings` / §15):** A aba ativa em configurações de comunidade é reconciliada dinamicamente contra a lista de abas autorizadas (`activeTab`), revertendo instantaneamente para `"general"` caso o usuário perca permissões administrativas (`manage_roles`, moderação) no meio da sessão.
