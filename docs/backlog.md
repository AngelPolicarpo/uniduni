...
de `replyToId` inexistente para `E_NOT_FOUND` e de alvo tombstonado para `E_MESSAGE_DELETED` (§31.7.4,
emenda 2026-09-05); (3) `dm.activate` agora executa `catchUp()` do projetor ao focar na conversa,
processando blocos acumulados em background; e (4) higiene criptográfica em `dmRuntime.close()`
com zeramento do buffer de `contentKey` retido. Duas lacunas de especificação registradas: **B78**
(transição de máquina de estados para retomar conversa em estado `left`) e **B79** (semântica de
`ackAhead` relativo à dobra vs comprimento total do log).

Atualizado antes em 2026-09-06 (§134). **§134** foi a verificação de um relatório
...
| B77 | **O link de mensagem que o produto copia não casa a gramática de §3.5.** `MessageActions` e `ChannelContextMenu` copiavam `p2p.app/m/...` sem esquema e com payload quebrado para canais. **§133 e §135 consertaram a interface**: os botões agora geram URLs válidas com protocolo (`https://${INVITE_LINK_HOST}/m/:code`), e o link de canal aponta pra primeira mensagem não lida (§8, 1.1.1 e §4) ou se oculta se vazio. A rota interna `/m/:code` do `MemoryRouter` consome e posiciona perfeitamente. O que resta para fechamento definitivo de protocolo é o deep link nativo (`comunidadep2p://m/<MSGREF>`): o núcleo devolver o MSGREF de uma mensagem ou o protocolo aceitar a rota com a referência decodificável | A decisão de contrato: o núcleo passa a devolver o MSGREF de uma mensagem (campo novo em `query.message`/`query.messages` de §15.6, ou comando próprio em §15.4), ou §3.5 aceita o formato resolvido pelo renderer. Definido isso, alinhar a forma copiada é do agente | §135, §133.6, §132.10, §3.5, §15.6 |
...
| B78 | **Retomada de conversa direta em estado `left` sem transição de máquina de estados definida.** §31.19 afirma que "recriar uma conversa é retomá-la", mas o manifest mantém a conversa em `state === 'left'` quando `dm.open` é invocado para um par esquecido. `#montar` abre o core local e appends locais são gravados em disco, mas a conversa fica isolada da rede — `autorizaDm` (§31.8) e o transporte (`dm.ts`) recusam e ignoram conexões com estado `left`. Requer emenda normativa definindo a transição de estado na retomada (`left` → `pending-out` ou `accepted`) e o restabelecimento da descoberta DHT | A transição da máquina de estados do manifest para uma conversa `left` retomada via `dm.open` ou `receberHello`. Definido o contrato normativo, a implementação cabe no serviço L2 | §31.19, §31.2 regra 4, §31.8, §135 |
| B80 | **A tela de falha de render não tem casa normativa.** A 1.0.1 acrescentou um limite de erro na raiz das telas (`frontend/src/components/shell/LimiteDeErro.tsx`): sem ele, uma exceção de render desmontava a árvore inteira e a janela ficava preta, sem mensagem e — por ser `frameless` — sem controles para fechar. Foi assim que o defeito de `null`→`{}` da IPC-R se apresentou a quem instalou a 1.0.0. O componente existe e foi decidido pelo operador; o que não existe é o texto: `frontend.md` não tem superfície para "a tela quebrou", e a cópia atual ("Esta tela quebrou", a mensagem do erro, "Tentar de novo" / "Recarregar a janela") foi escrita no ponto | O lugar e a redação em `frontend.md` — é superfície de UI, que é normativa. O componente e a colocação abaixo da `TitleBar` já estão medidos | §15.2, `frontend.md` §7, `frontend/src/App.tsx` |
| B79 | **Semântica de `ackAhead` contra comprimento físico total do log do par.** Em `dmFold/index.ts:226`, `ackAhead` compara `ackRaw` contra `prev.sides[outro].length` (a quantidade de registros do par interpretados até aquele ponto na ordem canônica) e não contra o comprimento físico total conhecido do core do par. Em cenários de desempate por `ordKey` com logs intercalados, ACKs matematicamente legítimos podem disparar falsamente o indicador de ordem provisória | A definição de escopo de `ackAhead`: estritamente local à dobra ou relativo aos metadados de cabeças do core | §31.6, L-27, §135 |
